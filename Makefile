...
CLI_BIN ?= $(BIN_DIR)/subsurface-ssi-cli
GUI_BIN ?= $(BIN_DIR)/subsurface-ssi-gui

SAMPLE_FILE ?= $(PROJECT_DIR)/tests/testdata/sample_subsurface.xml
SAMPLE_INDEX ?= 1
SAMPLE_QR ?= $(BIN_DIR)/sample-dive.png

.PHONY: help doctor test build-cli build-gui run-cli-sample run-gui clean
	...
	if [ "$$os" = "linux" ]; then \
		...
		if command -v pkg-config >/dev/null 2>&1; then \
			...
			if [ -n "$$missing" ]; then \
				...
				echo "[HINT] Ubuntu/Debian: sudo apt install libx11-dev libxrandr-dev libxi-dev libxcursor-dev libxinerama-dev libxxf86vm-dev libgl1-mesa-dev"; \
			else \
...
build-cli:
	mkdir -p $(BIN_DIR)
	cd $(PROJECT_DIR) && CGO_ENABLED=0 $(GO) build -ldflags "$(LDFLAGS)" -o ./bin/subsurface-ssi-cli ./cmd/cli

build-gui:
	mkdir -p $(BIN_DIR)
	cd $(PROJECT_DIR) && $(GO) build -ldflags "$(LDFLAGS)" -o ./bin/subsurface-ssi-gui ./cmd/app
